...
(friend hosting). One is fixed, two are instrumented and waiting on a repro, and
one is deferred by choice.
...
- [ ] If still ambiguous, flip each `DiagnosticFlags` bool in turn, rebuild,
      rejoin, and note whether the symptoms disappear.
...
### 4. Issue 3 — Per-player item pools (deferred by choice)

Every player currently rolls from the **host's** pool. Design notes live in the
comment block at the bottom of `Patches/pickupPatches.cs`.

- [ ] Key roulette state per `NetworkConnection` (`pp.Owner`) instead of one
      global static list.
- [ ] Stop `RouletteState.Reset()` wiping the pool from every
      `PlayerPickup.Awake()` — that fires for every player every round.
      **Leave it in place until A2 is settled**; it is an active diagnostic.
- [ ] Replace the unguarded, order-dependent `unowned_items[30]` starter index
      with a `SpawnerManager.NameToWeaponDict` lookup — also the natural seam for
      a future Archipelago `GrantByName` hook.
- [ ] Implementation note: this mod builds without FishNet's codegen weaver, so
      new `[ServerRpc]` methods are unavailable. `ClientManager.Broadcast<T>` /
      `ServerManager.RegisterBroadcast<T>` works, but `GenericWriter<T>.Write`
      and `GenericReader<T>.Read` must be assigned manually at startup.
...
- The O and P debug keybinds live in the `PlayerPickup.Update` Harmony patch, so
  they only respond in-game, not in the menu. Keeping them.
